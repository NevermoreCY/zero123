...
def main():
    ...
    for i in tqdm(range(len(folder_list))):
        ...
        # prompt_path = image_path = folders + '/' + folder + '/' + 'BLIP_best_text_v2.txt'
        prompt_path = folders + '/' + folder + '/' + 'BLIP_best_text_v2.txt'
        with open (prompt_path,'r') as f:
            prompt = f.readline()

        if prompt not in data_distribution:
            data_distribution[prompt] = [folder]
        else:
    ...
    for key in data_distribution:
        print(key, len(data_distribution[key]))
